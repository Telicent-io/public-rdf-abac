...
public class LabelsSet {
    ...
    // Decision = set default.
    //Decision = DEFAULT

    /** If there are no labels in the labels set (labels.get(triple) == null). */
    private static Decision DefaultChoiceNoLabels = Decision.ALLOW;

    /** If there is an empty set of labels for a triple (labels.get(triple) == {}) */
    private static Decision DefaultChoiceNoAttributesForTriple = Decision.ALLOW;

    /** LabelsSet wide default. e.g. {@link Decision.NONE} */
    private static Decision DefaultLabelsSet = Decision.ALLOW;
}
